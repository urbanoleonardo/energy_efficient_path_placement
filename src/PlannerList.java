...
public class PlannerList implements Callable<EnergyPoint> {
	...
	private LinkedList<LinkedList<Configuration>> thetaMList;
	...
	public PlannerList(Robot robot)
	{
	...
	public PlannerList(Target[] targets, Robot robot)
	{
	...
	public PlannerList(Target target1, Target target2, Robot robot)
	{
	...
	public PlannerList(Target target1, Target target2, Robot robot, Point3D point3D)
	{
	...
	public PlannerList (Path p, Robot robot){
...
//				System.out.println("x: " + point.getX() + " y : " + point.getY() + " z : " + point.getZ());
	...
	private boolean solver(Path inputPath){
		...
		thetaMList = new LinkedList<LinkedList<Configuration>>();
		...
		for(int j = 0; j < trajectoryLength; j++){
			...
			error = onlineInvKinematics(pathTrajectory.points.get(j));
		...
		int[] solutionVector = new int[thetaMList.size()];
//		System.out.println("Size of ThetaListM " + thetaMList.size());
//		System.out.println("Size of solutionVector " + solutionVector.length);
		ArrayList<int[]> Solutions = new ArrayList<int[]>();
		configIteration(0, 0, pathTrajectory.timeInstants,solutionVector, Solutions);
...
//		System.out.println("Time for the whole procedure: " + (time4-time1)/1E9);
	...
	private int onlineInvKinematics(Target nextPosition)
	{
		/*
		...
		LinkedList<Configuration> singleThetaMList = new LinkedList<Configuration>();
		Configuration configuration = new Configuration();

		for(int i = 0; i < theta1.length; i++){
			configuration.setJointValue(1, theta1[i]);
			...
			for(int j = 0; j < theta23.length; j++){
				...
				configuration.setJointValue(2, theta2);
				configuration.setJointValue(3, theta3);
				double[][] theta_456 = solveTheta456(nextPosition, theta1[i], theta2, theta3); 

				for(int k = 0; k < theta_456[0].length; k++){

					configuration.setJointValue(4,  theta_456[0][k]);
					configuration.setJointValue(5, -theta_456[1][k]);
					configuration.setJointValue(6,  theta_456[2][k]);

					List<Double> list = configuration.getJointValues();

					Configuration newConfiguration = new Configuration(list);

					singleThetaMList.add(newConfiguration);
		...
		//CHECK limitation
		this.applyLimitation(singleThetaMList);

		this.thetaMList.add(singleThetaMList);
...
//			System.out.println("x: " + this.point3D.getX() + " y : " + this.point3D.getY() + " z : " + this.point3D.getZ());
			...
			for(int i = 0; i < prevTheta.length; i++){
				//				System.out.println("Size : " + this.thetaMList.getFirst().get(firstConf).size());
				prevTheta[i] = this.thetaMList.getFirst().get(firstConf).getJointValue(i+1);
			} 
			...
			for(int i = 1 ; i < N; i++) //i should start from 1----- i from 1 to 134
			{
				...
				Configuration initConf = this.thetaMList.get(i).get(index);  
				for(int j = 0; j < 6; j++){ 
					theta[j] = initConf.getJointValue(j+1);
					} //there was a +1 - WHY?
				...
				//THIS LINE WILL PREVENT FROM CALCULATING OTHER SOLUTIONS!
				if(energy > minEnergy){
					break;
				}
...
//		System.out.println("Last sample of inverse dynamics took : " + (time2 - time1)/1E9 );
	...
	private double[][] solveTheta456(Target target_position, double theta_1, double theta_2, double theta_3){
	...
	private double theta4Optimal(double theta5, double theta1, double theta2, double theta3){
		/*
		...
		double temp = 0;
		double theta4Optimal = 0;

		LinkedList<Configuration> lastConfigurationList = this.thetaMList.getLast();


		if (theta5 < 0){
			for( Configuration config : lastConfigurationList){
				if(config.getJointValue(5) < 0){
					//theta_sing=[theta_sing,abs(thetaM(1:3,i,t-1)-[theta1;theta2;theta3])];
					temp = Math.abs(config.getJointValue(1) - theta1) + Math.abs(config.getJointValue(2) - theta2) + Math.abs(config.getJointValue(3) - theta3);
					if(temp < minSum){
						minSum = temp;
						theta4Optimal = config.getJointValue(4);
					}
				}
		...
		return theta4Optimal;
	}

	private void applyLimitation(LinkedList<Configuration> singleThetaMList)
	{

		for(Iterator<Configuration> configIterator = singleThetaMList.iterator(); configIterator.hasNext();){
			Configuration currConfig = configIterator.next();
			int i = 0;
			//			for(double jointValue : currConfig.getJointValues()){
			for(Iterator<Double> jointIterator = currConfig.getJointValues().iterator(); jointIterator.hasNext();){
				double jointValue = jointIterator.next();
				if(jointValue < this.jointLimits[i][0] || jointValue > this.jointLimits[i][1])
				{
	...
	private void configIteration(int pointi, int prevConfig, ArrayList<Double> trajectoryTimes, int[] currentSolution, ArrayList<int[]> solutions){
		/*
		...
		for(Configuration currConfig : thetaMList.get(pointi)){
			...
			{
				error = checkConfiguration(currentConfigIndex, prevConfig, pointi, trajectoryTimes);
			}
			...
			if(error == 0) 
			{
				...
				currentSolution[pointi] = currentConfigIndex;
				if (pointi < N-1 )//N-2 or N-1?? pointi assumes values 0:134=N-1
				{
					configIteration(pointi+1, currentConfigIndex, trajectoryTimes, currentSolution, solutions);
				}
				else
				{
...
//										System.out.println("New solution found, the number of solutions is : " + solutions.size());
	...
	private int checkConfiguration(int currConfig, int prevConfig, int point, ArrayList<Double> trajectoryTimes){
		int error = 0;
		...
		Configuration currentConfig = this.thetaMList.get(point).get(currConfig);
		Configuration previousConfig = this.thetaMList.get(point-1).get(prevConfig);

		for(int i = 0; i < currentConfig.getJointValues().size(); i++){
			double currJointValue = currentConfig.getJointValue(i+1);
			double prevJointValue = previousConfig.getJointValue(i+1);
			deltaJoint = Math.abs(currJointValue - prevJointValue);

			if((i <= 2 || i == 4) && (deltaJoint > thAssump[i]))
			{
				return error = 1;
			}

			if((i == 3 || i == 5) && (deltaJoint > thAssump[i]))
			{

				double delta_jointp = Math.abs(currJointValue + 2*Math.PI - prevJointValue);
				double delta_jointm = Math.abs(currJointValue - 2*Math.PI - prevJointValue);

				if(delta_jointp < thAssump[i]) { 
					LinkedList<Configuration> newConfList = this.thetaMList.get(point);
					currentConfig.setJointValue(i+1, currJointValue + 2*Math.PI);
					newConfList.set(currConfig, currentConfig);
					this.thetaMList.set(point, newConfList);
				} else {

					if(delta_jointm < thAssump[i]){
						LinkedList<Configuration> newConfList = this.thetaMList.get(point);
						currentConfig.setJointValue(i+1, currJointValue - 2*Math.PI);
						newConfList.set(currConfig, currentConfig);
						this.thetaMList.set(point, newConfList);
	...
	public void displayThetaM(int index){
		...
		for(int j = 0; j < thetaMList.get(index).get(0).size(); j++){
			for(int k = 0; k < thetaMList.get(index).size(); k++){
				System.out.print(thetaMList.get(index).get(k).getJointValue(j+1) + " ");
			}
			System.out.println(" ");
		}
	...
	public LinkedList<LinkedList<Configuration>> getThetaMList(){
		return this.thetaMList;
	}
